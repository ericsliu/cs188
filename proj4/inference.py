...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        particleCount = 0
        self.beliefs = list()
        particlesPerPosition = self.numParticles / float(len(self.legalPositions))
        counter = 1
        for p in self.legalPositions:
            while particleCount < float(particlesPerPosition) * counter:
                self.beliefs.append(p)
                particleCount += 1
            counter += 1
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions: newDist[p] = 0.0
        newBeliefs = list()
        counter = 0
        distSum = 0.0
        if noisyDistance is None:
            while counter < self.numParticles:
                newBeliefs.append(self.getJailPosition())
                counter += 1
            distSum = 1.0
        else:
            for belief in self.beliefs:
                distance = util.manhattanDistance(belief, pacmanPosition)
                newDist[belief] += emissionModel[distance]
            newDist.normalize()
            distSum = 0.0
            ...
            if distSum > 0.0:
                while counter < self.numParticles:
                    newBeliefs.append(util.sample(newDist))
                    counter += 1
        if distSum == 0.0:
            self.initializeUniformly(gameState)
        else:
    ...
    def elapseTime(self, gameState):
        ...
        newBeliefs = list()
        for belief in self.beliefs:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, belief))
    ...
    def getBeliefDistribution(self):
        ...
        for p in self.legalPositions: distribution[p] = 0.0
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        newDists = [util.Counter for ghost in self.ghostAgents]
        for p in self.legalPositions:
