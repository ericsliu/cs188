...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        #Most likely position of ghosts
        bestPositions = []
        for i in range(0, len(livingGhostPositionDistributions)):
            best = (None, 0)
            for position, distribution in livingGhostPositionDistributions[i].items():
                if distribution > best[1]:
                    best = (position, distribution)
            bestPositions.append((i,best)) # (ghostNum, (location, distribution))

        #Pick the closest ghost
        closestGhost = (None, -1) #(location, distance)
        for j in range(0, len(bestPositions)):
            distanceTo = self.distancer.getDistance(pacmanPosition, bestPositions[j][1][0])
            if (closestGhost[0] is None) or (distanceTo < closestGhost[1]):
                closestGhost = (bestPositions[j][1][0], distanceTo)

        #Pick best action to that ghost
        closestAction = (None, 0) #(action, newDistance)
        for action in legal:
            newDistance = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), closestGhost[0])
            if (closestAction[0] is None) or (newDistance < closestAction[1]):
                closestAction = (action, newDistance)
        return closestAction[0]